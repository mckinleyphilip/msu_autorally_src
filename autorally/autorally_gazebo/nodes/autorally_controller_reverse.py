  ...
  """
    ...
    def __init__(self, namespace='controller_manager'):
      ...
      # Shock absorbers
        ...
        try:
            ...
            rospy.logwarn('The specified list of shock absorbers is invalid. No shock absorbers will be used.'
                          )
      ...
      # divided by two.
        ...
        self._wheelbase = numpy.linalg.norm(front_cent_pos
                - rear_cent_pos)
      ...
      # load chassis commander priorities
        ...
        self.commandPriorities = \
            rospy.get_param('~chassisCommandProirities', [])
        self.commandPriorities = sorted(self.commandPriorities.items(),
                key=operator.itemgetter(1))
        rospy.loginfo('AutoRallyGazeboController: Loaded %d chassis commanders'
      ...
      # Publishers and subscribers
        ...
        self._left_steer_cmd_pub = _create_cmd_pub(list_ctrlrs,
                left_steer_ctrlr_name)
        self._right_steer_cmd_pub = _create_cmd_pub(list_ctrlrs,
                right_steer_ctrlr_name)

        self._left_front_axle_cmd_pub = \
            _create_axle_cmd_pub(list_ctrlrs,
        ...
        (self.left_front_name, self.left_front_dia) = \
            self.getJointStateWheelParams('left', 'front')
        (self.right_front_name, self.right_front_dia) = \
            self.getJointStateWheelParams('right', 'front')
        (self.left_rear_name, self.left_rear_dia) = \
            self.getJointStateWheelParams('left', 'rear')
        (self.right_rear_name, self.right_rear_dia) = \
            self.getJointStateWheelParams('right', 'rear')

        self.wheelSpeedFront = 0.0

      # don't set up callback until params are initialized

        self.wheelSpeedsPub = rospy.Publisher('/wheelSpeeds',
                wheelSpeeds, queue_size=1)

      # self.sub = rospy.Subscriber('/autorally_platform/gazebo/link_states', ModelStates, self.callback)

        self.chassisStatePub = rospy.Publisher('/chassisState',
                chassisState, queue_size=1)

        self.chassisCmdSub = dict()
        for (cmd, priority) in self.commandPriorities:
            self.chassisCmdSub[cmd] = rospy.Subscriber('/' + cmd
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self._cmd_timeout > 0.0 and t - self._last_cmd_time \
                > self._cmd_timeout:
        ...
        # vehicle.
                ...
                (steer_ang_changed, center_y) = \
                    self._ctrl_steering(self._last_steer_ang, 0.0,
                        0.001)
                self._ctrl_axles(0.0, 0.0, 0.0, steer_ang_changed,
                                 center_y)
            elif delta_t > 0.0:
            ...
            # rospy.logwarn("looking for chassis commander %s with priority %d", cmd, priority)
                        ...
                        if cmd in self.chassisCmds:
                            if abs(self.chassisCmds[cmd].steering) \
                                <= 1.0 and rospy.Time.now() \
                                - self.chassisCmds[cmd].header.stamp \
                                < rospy.Duration.from_sec(0.2) \
                                and not foundSteering:

                # rospy.loginfo("%s in control of steering", cmd);

                                steer_ang = -math.radians(25) \
                                    * self.chassisCmds[cmd].steering
                                steer_ang_vel = 0.0
                                chassisSt.steering = \
                                    self.chassisCmds[cmd].steering
                                chassisSt.steeringCommander = \
                                    self.chassisCmds[cmd].sender
                                foundSteering = True

                            if abs(self.chassisCmds[cmd].throttle) \
                                <= 1.0 and rospy.Time.now() \
                                - self.chassisCmds[cmd].header.stamp \
                                < rospy.Duration.from_sec(0.2) \
                                and not foundThrottle:

                # rospy.loginfo("%s in control of throttle", cmd);

                                if self.chassisCmds[cmd].throttle \
                                    >= 0.0:
                                    speed = self.rear_axle_max_effort \
    * self.chassisCmds[cmd].throttle
                                else:
                                    speed = self.rear_axle_brake_effort \
    * self.chassisCmds[cmd].throttle

                                accel = 0.0
                                chassisSt.throttle = \
                                    self.chassisCmds[cmd].throttle
                                chassisSt.throttleCommander = \
                                    self.chassisCmds[cmd].sender
                                foundThrottle = True

                            if self.chassisCmds[cmd].frontBrake >= 0.0 \
                                and self.chassisCmds[cmd].frontBrake \
                                <= 1.0 and rospy.Time.now() \
                                - self.chassisCmds[cmd].header.stamp \
                                < rospy.Duration.from_sec(0.2) \
                                and not foundFrontBrake:

                # the brake acts to slow any movement

                                frontBrake = \
                                    numpy.sign(self.wheelSpeedFront) \
                                    * (-self.front_axle_brake_effort
                                        * self.chassisCmds[cmd].frontBrake)
                                chassisSt.frontBrake = \
                                    self.chassisCmds[cmd].frontBrake
                                chassisSt.frontBrakeCommander = \
                                    self.chassisCmds[cmd].sender
                    ...
                    (steer_ang_changed, center_y) = \
                        self._ctrl_steering(steer_ang, steer_ang_vel,
                            delta_t)
                    self._ctrl_axles(speed, accel, delta_t,
                            steer_ang_changed, center_y)
    ...
    # and inverse of the circumference.
        ...
        steer_link_name = rospy.get_param(prefix + 'steering_link_name'
                , side + '_steering_link')
        steer_ctrlr_name = rospy.get_param(prefix
                + 'steering_controller_name', side
                + '_steering_controller')
        (axle_ctrlr_name, inv_circ) = \
            self._get_common_wheel_params(prefix)
    ...
    # axle controller name, and inverse of the circumference.
        ...
        link_name = rospy.get_param(prefix + 'link_name', side
                                    + '_wheel')
        (axle_ctrlr_name, inv_circ) = \
            self._get_common_wheel_params(prefix)
    ...
    # circumference.
        ...
        axle_ctrlr_name = rospy.get_param(prefix
                + 'axle_controller_name', None)
    ...
    # rear wheel link.
        ...
        while True:
            try:
                (trans, not_used) = \
                    tfl.lookupTransform(self._right_rear_link_name,
                        link, rospy.Time(0))
    ...
    # Compute theta, the virtual front wheel's desired steering angle.
      ...
      # Limit the steering velocity.
            ...
            ang_vel = max(-steer_ang_vel_limit, min(ang_vel,
                          steer_ang_vel_limit))
    ...
    # Compute the desired angular velocities of the wheels.
      ...
      # Front
            ...
            self._left_front_ang_vel = gain * r \
                * self._left_front_inv_circ
            r = math.sqrt(right_dist ** 2 + self._wheelbase_sqr)
            self._right_front_ang_vel = gain * r \
                * self._right_front_inv_circ

      # Rear

            gain = 2 * pi * veh_speed / center_y
            self._left_rear_ang_vel = gain * left_dist \
                * self._left_rear_inv_circ
            self._right_rear_ang_vel = gain * right_dist \
                * self._right_rear_inv_circ
    ...
    # print data
        ...
        ws.lfSpeed = self.getWheelSpeed(data, self.left_front_name,
                self.left_front_dia)
        ws.rfSpeed = self.getWheelSpeed(data, self.right_front_name,
                self.right_front_dia)
    ...
    # published speeds can't be negative so data mimics the physical platform
        ...
        ws.lbSpeed = abs(self.getWheelSpeed(data, self.left_rear_name,
                         self.left_rear_dia))
        ws.rbSpeed = abs(self.getWheelSpeed(data, self.right_rear_name,
                         self.right_rear_dia))
    ...
    def getJointStateWheelParams(self, lr_prefix, fr_prefix):
        dia = \
            rospy.get_param('/autorally_platform/autorally_controller/'
                            + lr_prefix + '_' + fr_prefix
                            + '_wheel/diameter')
    ...
    def getLinkStateFrontWheelParams(self, lr_prefix):
        dia = \
            rospy.get_param('/autorally_platform/autorally_controller/'
                            + lr_prefix + '_front_wheel/diameter')
        name = 'autorally_platform::' \
            + rospy.get_param('/autorally_platform/autorally_controller/'
                               + lr_prefix
                              + '_front_wheel/steering_link_name')
        return (name, dia)

    def getLinkStateRearWheelParams(self, lr_prefix):
        dia = \
            rospy.get_param('/autorally_platform/autorally_controller/'
                            + lr_prefix + '_rear_wheel/diameter')
        name = 'autorally_platform::' \
            + rospy.get_param('/autorally_platform/autorally_controller/'
