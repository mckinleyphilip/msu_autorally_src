  ...
  """
	...
	def spin(self):
		...
		# vehicle.
				...
				with self.chassisCmdLock:
					...
					for (cmd, priority) in self.commandPriorities:
						...
						if cmd in self.chassisCmds:
                            ...
                            # Find steering commad
							if abs(self.chassisCmds[cmd].steering) <= 1.0 and rospy.Time.now() - self.chassisCmds[cmd].header.stamp < rospy.Duration.from_sec(0.2) and not foundSteering:
                            ...
                            # Find Throttle command
							if abs(self.chassisCmds[cmd].throttle) <= 1.0 and rospy.Time.now() - self.chassisCmds[cmd].header.stamp < rospy.Duration.from_sec(0.2) and not foundThrottle:
								...
								chassisSt.reverse = self.chassisCmds[cmd].reverse # Set the reverse flag dependent on the throttle commander //new//
								foundThrottle = True



                            # Find brake command
							if self.chassisCmds[cmd].frontBrake >= 0.0 and self.chassisCmds[cmd].frontBrake <= 1.0 and rospy.Time.now() - self.chassisCmds[cmd].header.stamp < rospy.Duration.from_sec(0.2) and not foundFrontBrake:
				...
				# If the reverse flag is set in the chassisState, reverse the throttle //new//
				if (chassisSt.reverse == True):
					speed *= -1
	...
	# published speeds can't be negative so data mimics the physical platform
		...
		ws.lbSpeed = self.getWheelSpeed(data, self.left_rear_name,
						 self.left_rear_dia)
		ws.rbSpeed = self.getWheelSpeed(data, self.right_rear_name,
						 self.right_rear_dia)
