...
class AutoRallyCtrlr(object):
    ...
    def __init__(self, namespace="controller_manager"):
        ...
        rospy.on_shutdown(self.on_shutdown) # note that this method does nothing... -JF

        # Parameters

        # Defaults
        self._DEF_WHEEL_DIA = 0.19  # Default wheel diameter. Unit: meter.
        self._DEF_EQ_POS = 0.0  # Default equilibrium position. Unit: meter.
        ...
        while not rospy.is_shutdown():
            ...
            if self._cmd_timeout > 0.0 and t - self._last_cmd_time > self._cmd_timeout:
                ...
                with self.chassisCmdLock:
                        ...
                        if cmd in self.chassisCmds:
                            ...
                            if abs(self.chassisCmds[cmd].steering) <= 1.0 and \
                                (rospy.Time.now()-self.chassisCmds[cmd].header.stamp) < \
                                 rospy.Duration.from_sec(0.2) and not foundSteering:
                                #rospy.loginfo("%s in control of steering", cmd);
                                # -JF-
                                # Changes max angle from 25 -> 21 (as this is mentioned in
                                # xacro file)
                                # Also removed negation to make it more straightforward when
                                # applying cmds
                                # -JF-
                                steer_ang = math.radians(21)*self.chassisCmds[cmd].steering
                                steer_ang_vel = 0.0
                            ...
                            if abs(self.chassisCmds[cmd].throttle) <= 1.0 and \
                                ...
                                # -JF-
                                # note that the "speed" var is passed to another method which
                                # attempts to change current speed to that speed
                                # limited by accel_limit and then sets the left and right rear
                                # wheel speed vars
                                # -JF-
                    ...
                    # -JF-
                    #   o Moved this from line 406 so that reverse is applied before _ctrl_steering
                    #     and _ctrl_axels (which set _theta_left, _theta_right, and ang velocities for
                    #     for rear wheels).
                    #   o also added line to flip steering angle as well
                    # -JF-
                    if (chassisSt.reverse):
                        speed *= -1
                        steer_ang *= -1
    ...
    def _ctrl_steering(self, steer_ang, steer_ang_vel_limit, delta_t):
        ...
        if steer_ang_changed:
            ...
            self._theta_left = _get_steer_ang(math.atan(self._inv_wheelbase *\
                                         (center_y - self._joint_dist_div_2)))
            self._theta_right = _get_steer_ang(math.atan(self._inv_wheelbase *\
                                       (center_y + self._joint_dist_div_2)))
    ...
    def wheelSpeedsCb(self, data):
        ...
        # This used to be what this module published, changed it back to above -JF
    ...
    def getJointStateWheelParams(self, lr_prefix, fr_prefix):
        dia = rospy.get_param("/autorally_platform/autorally_controller/"
    ...
    def getLinkStateFrontWheelParams(self, lr_prefix):
        ...
        name = "autorally_platform::" + rospy.get_param("/autorally_platform/autorally_controller/"
